...
class mSistem extends CI_Model {
	...
	*/
	...
	public function hapus_include($contest_id, $problem_id, $username = '') {
		$db2 = $this->load->database('grouptwo', TRUE);
		$db2->select('submissions.submit_content, users.username, submissions.score, submissions.submitted_time, submissions.grade_content');
		$db2->from('submissions');
		$db2->join('users', 'submissions.submitter_id = users.id');
		$db2->where('submissions.contest_id', $contest_id);
		$db2->where('submissions.problem_id', $problem_id);
		...
		foreach ($query->result_array() as $row) {
			...
			$line = str_replace('/#include <stdio.h>/', '', $line);
			//$cmd = 'gcc -o '.$line;
			$output = shell_exec("gcc -E /home/grena/Desktop/dummyprogram/sorting/cluster_1_2/A14.c");
			echo "$output = ".$output;
			if($output)
				echo "yes";
			else 
				echo "no";
			$file[$row['username']] = $output;
		}
		return $file;
	}
